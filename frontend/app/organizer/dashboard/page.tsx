            ...
            <aside className="w-60 flex flex-col h-screen sticky top-0 border-r border-white/[0.06]" style={{ background: '#100e1a' }}>
                ...
                <div className="mt-auto border-t border-white/[0.06]">
                    <div className="px-4 pt-4 pb-1">
                        ...
                        <button className="w-full flex items-center gap-3 px-3 py-2 text-[13px] text-cream-warm/40 hover:text-cream-warm/80 hover:bg-white/[0.04] transition rounded-md">
...
function PageantsSection({ user }: { user: User }) {
    ...
    const [form, setForm] = useState({ name: "", country: "", province: "", city: "", barangay: "", zip: "", date: "" });
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setShowModal(false);
            setForm({ name: "", country: "", province: "", city: "", barangay: "", zip: "", date: "" });
                            ...
                            <tbody className="divide-y divide-white/[0.04]">
                                {candidates.map((c) => (
                                    <tr key={c.id} className="hover:bg-white/[0.02] transition">
                                        ...
                                        <td className="px-5 py-3.5 text-cream-warm/50">{c.email || "—"}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="border border-white/[0.08] rounded-xl p-8 max-w-md w-full shadow-2xl" style={{ background: '#100e1a' }}>
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            ...
                            <div>
                                <label className="text-[9px] uppercase tracking-[0.18em] text-cream-warm/40 block mb-1.5 font-semibold">Email</label>
